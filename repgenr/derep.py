...
### INFO
# install dRep:
#mamba create -n DREP python=3.9 drep -y
# Must install checkm manually:
#mamba install checkm-genome
# Must custom-export path:
#conda env config vars set CHECKM_DATA_PATH=/data/users/jaclew/software/anaconda3/envs/DREP/checkm_data
# Must get libgsl.so.25 (required; for Mash-software) but we only have libgsl.so.27 in installation (I tried dirty solution: softlink libgsl.so.27 as libgsl.so.25, if it doesnt work I will gopy libgsl.so.25 from another package into the DREP environment [/data/users/jaclew/software/anaconda3/envs/DREP/lib])

# for genome-script (download): ncbi datasets is needed:
#mamba install ncbi-datasets-cli

# for phylogeny using mashtree
