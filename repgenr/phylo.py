...
if run_mode == 'accurate':
    ...
    with open(phylo_wd+'/'+'input.list','w') as nf:
        ...
        for enum,file_ in enumerate(sorted(os.listdir(genome_files_dir))):
            ...
            if enum == 0:
                ...
                continue
            #/
            nf.write(file_.replace('.fasta','')+'\t'+genome_files_dir+'/'+file_+'\n')
        #/
        ...
        if not skip_outgroup:
            ...
            nf.write(outgroup_file.replace('.fasta','')+'\t'+workdir+'/outgroup/'+outgroup_file)
            #/
        #/
    ###/
    ...
    with open(phylo_wd+'/'+'core.full.aln.treefile','r') as f:
        with open(output_file_path,'w') as nf:
            for line in f:
                nf.write(line.replace('Reference:',snippy_ref.replace('.fasta','')+':'))
    ###/
####/
